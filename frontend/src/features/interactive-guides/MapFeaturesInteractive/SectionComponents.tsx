...
import { stepsSecondHalf, proTips } from './stepsDataContinued';
...
export const GuideHeader: React.FC = () => (
  <div className="mb-8">
    <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-3">
      ...
      <span>Map Features & Controls Interactive Guide</span>
    </h1>
    <p className="text-lg text-gray-600 dark:text-gray-300">
      Learn how to use all map controls, zoom features, layer management,
      and search functionality. This guide covers 9 essential map features
      for effective GIS navigation.
    </p>
  </div>
);
...
export const StepCard: React.FC<StepCardProps> = ({ step, index, isExpanded, onToggle }) => {
  ...
  return (
    <div
      className={`${colors.bg} ${colors.border} border-2 rounded-xl overflow-hidden transition-all duration-300 hover:shadow-lg`}
    >
      {/* Step Header */}
      <button
        onClick={onToggle}
      ...
      >
        <div className="flex-shrink-0">
          <div
            className={`w-16 h-16 ${colors.bg} rounded-full flex items-center justify-center text-3xl border-2 ${colors.border}`}
          >
            {step.icon}
          </div>
        </div>
