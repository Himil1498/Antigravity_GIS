...
const DeleteConfirmationDialog: React.FC<DeleteConfirmationDialogProps> = ({
  ...
  const getBgColor = () => {
    if (type === "danger") return "bg-red-100 dark:bg-red-900";
    if (type === "warning") return "bg-yellow-100 dark:bg-yellow-900";
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto">
      ...
      <div className="flex min-h-full items-center justify-center p-4">
        <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full p-6 transform transition-all">
          ...
          {/* Icon */}
          <div
            className={`mx-auto flex h-12 w-12 items-center justify-center rounded-full ${getBgColor()}`}
          >
            {getIcon()}
          </div>
            ...
            {itemCount && itemCount > 1 && (
              <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3 mb-4">
                <p className="text-sm font-medium text-red-800 dark:text-red-300">
                  <span className="font-bold">{itemCount}</span> items will be
                  deleted
                </p>
              </div>
            )}
