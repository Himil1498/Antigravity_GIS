...
const getUserRegions = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to get user regions" });
  }
};
...
const assignRegion = async (req, res) => {
  try {
    ...
    if (!regionId) {
      return res
        .status(400)
        .json({ success: false, error: "Region ID required" });
    }
    ...
    await pool.query(
      ...
      [id, regionId, accessLevel, req.user.id],
    );
    ...
    try {
      ...
      console.log(
        `📧 User ${userName} notified about region assignment: ${regionName}`,
      );
    } catch (notifError) {
...
const unassignRegion = async (req, res) => {
  try {
    ...
    try {
      ...
      console.log(
        `📧 User ${userName} notified about region removal: ${regionName}`,
      );
    } catch (notifError) {
      console.error("Failed to send notification to user:", notifError);
    }
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to unassign region" });
  }
};
