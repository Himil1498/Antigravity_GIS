...
class CatalogService {
  ...
  async getVectorTiles(
    ...
    try {
      ...
      const result = await prisma.$queryRawUnsafe(query, ...(queryParams || []));
      // Prisma (PrismaPg adapter) returns bytea as Uint8Array, not Buffer.
      // Express res.send() and the frontend MVT decoder require a Node Buffer.
      const mvt = result?.[0]?.mvt || null;
  ...
  async addManualFeature(userId, payload) {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      const folderResult = await tx.$queryRawUnsafe(
      ...
      const fileResult = await tx.$queryRawUnsafe(
      ...
      if (fileResult.length > 0) {
        ...
        const insertFileResult = await tx.$queryRawUnsafe(
      ...
      const insertFeatureResult = await tx.$queryRawUnsafe(
      ...
      return { success: true, featureId: insertFeatureResult[0].id };
    });
  }
  ...
  async softDeleteFeature(featureId, userId, deleteLinkedReports = false) {
    return await prisma.$transaction(async (tx) => {
      ...
      const totalDeleted = 1 + deleteLinksResult.length;
  ...
  async restoreItem(itemId, itemType, userId) {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      if (itemType === 'folder' || itemType === 'file') {
        ...
        const featureInfo = await tx.$queryRawUnsafe(
  ...
  async permanentDeleteItem(itemId, itemType) {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      if (itemType === 'file') {
         ...
         const storageQuery = await tx.$queryRawUnsafe("SELECT storage_path FROM network_files WHERE id = $1", [itemId]);
         const storagePath = storageQuery[0]?.storage_path;
         if (storagePath) {
      ...
      await tx.$queryRawUnsafe(`DELETE FROM ${table} WHERE id = $1`, [
        itemId,
      ]);
  ...
  async emptyRecycleBin(userId, userRole) {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      const filesResult = await tx.$queryRawUnsafe(fileQuery, params);
